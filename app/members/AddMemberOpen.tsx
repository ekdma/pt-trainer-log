...
export default function AddMemberOpen({ onClose, onMemberAdded }: Props) {
  ...
  const fetchPackages = async () => {
    const { data, error } = await supabase
      .from('packages')
      .select('*')
      .order('valid_date, package_name, pt_session_cnt, group_session_cnt', { ascending: true }) // 오름차순 정렬
        ...
        {isPackagePopupOpen && (
          <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white w-[360px] max-h-[500px] overflow-y-auto p-5 rounded-xl shadow-xl border">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">등록할 패키지를 선택하세요</h3>
              ...
              <div className="space-y-3">
                {packages.map((pkg) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-sm text-gray-700">
                      PT: <span className="font-medium">{pkg.pt_session_cnt}회</span> / 그룹: <span className="font-medium">{pkg.group_session_cnt}회</span>
                    </div>
                    <div className="mt-1 text-sm text-gray-700">
              ...
              <div className="text-right mt-5">
                <Button size="sm" variant="outline" className="px-4 py-2 text-sm" onClick={() => setIsPackagePopupOpen(false)}>
                  닫기
                </Button>
              </div>
            </div>
