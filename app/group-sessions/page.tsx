...
interface GroupSessionRaw {
    group_session_id: number;
    group_session_date: string;
    theme: string;
    trainer_id: number;
    trainers: { name: string }[]; // Supabase 응답은 배열
    workouts: { workout_name: string }[];
    participants: { members: { name: string } }[];
  }
...
export default function GroupSessionPage() {
    ...
    const fetchSessions = async () => {
        ...
        const transformed: GroupSession[] = data.map((session: any) => ({
            group_session_id: session.group_session_id,
            group_session_date: session.group_session_date,
            theme: session.theme,
            trainer_id: session.trainer_id,
            trainer_name: session.trainer?.name ?? '정보 없음', // ✅ 변경됨
            workouts: session.workouts?.map((w: any) => w.workout_name) ?? [],
            participants: session.participants?.map((p: any) => p.members?.name) ?? [],
        }))
